%% 使用FISTA的第一种线搜索方法和等步长梯度下降法并比较结果
% 参数说明：
...
% 调用FISTA_linreasch_1方法
[K, x_min, f_min, X, F, dF_norm] = FISTA_linreasch_1_method([-10; -1], 1e-3,A,b,0.33);
disp(['利用FISTA_linreasch_1方法: 迭代次数 = ', num2str(K), ', 最优值点 x1 = ', num2str(x_min(1)), ', x2 = ', num2str(x_min(2)), ', 最小值 = ', num2str(f_min)]);
...
% FISTA_linreasch_1 方法
function [K, x_min, f_min, X, F, dF_norm] = FISTA_linreasch_1_method(start, eps,A,b,rho)
    ...
    % 先跑一下
    r = 1;
    r = 1/( 0.5*(1 + sqrt(1 + 4/r) )  );

    [alpha,X(:, 2)] = line_search_1(alpha,rho,X(:, 1),A,b);
    ...
    while dF_norm(k) >= eps && k < max_iter
        ...
        [alpha,X(:, k + 1)] = line_search_1(alpha,rho,Y,A,b);
...
function [alpha_new,X] = line_search_1(alpha_old,rho,Y,A,b)
    alpha_new = alpha_old;
    X = Y - alpha_new * df(Y,A,b);
    while f(X,A,b) > f(Y,A,b) + (X-Y)'*df(Y,A,b) + norm(X-Y,2)^2/(2*alpha_new)
